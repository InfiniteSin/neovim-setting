...
- **LSP:** native `vim.lsp` only. Enabled servers (in `lua/plugins/lsp.lua`): `ast_grep`, `lua_ls`, `clangd`, `ruff`, `ty`, `debugpy`, `marksman`, `mdx_analyzer`. `lua_ls` gets an extended local config there (declares the `vim` global, LuaJIT runtime, nvim runtime files + luv types in the workspace library, telemetry off, and a `root_markers` list with a `vim.fn.has('nvim-0.11.3')` compatibility branch). For Markdown/MDX, `vim.filetype.add` maps `.mdx` to `markdown.mdx` (marksman matches it natively; core treesitter maps it to the `markdown` parser) and `mdx_analyzer`'s vendored `filetypes = { 'mdx' }` is overridden to `{ 'markdown.mdx' }`. Server binaries must exist on `PATH` (install via Mason or manually).
